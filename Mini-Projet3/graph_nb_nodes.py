...
filename = "nb_nodes_data.txt" #Nom du fichier qui permet de réaliser les graphiques

out_name = "nb_nodes" #Fichiers de sortie contenant ce nom, ainsi qu'un sous-dossier de ce nom
...
def heatmap_builder(size, drange, title, output, data):
	...
	for t in data:
		tab_tmp += t
	uniform = Series(random.random(size))
	...
	pyplot.savefig(output)
#TIME
graph_builder("Samples", "Time in iteration", "Time graph", 1, 10000, f"{out_path}time_graph_{out_name}.pdf", tab_time, list(range(25, 400, 50)))
f9 = pyplot.figure(9)
histogram_builder("Time in iteration", "Density", "Time histogram", True, 10, f"{out_path}time_hist_{out_name}.pdf", tab_time, list(range(25, 400, 50)))
f8 = pyplot.figure(8)
heatmap_builder(200, list(range(0,10)), "Time heatmap", f"{out_path}time_heat_{out_name}.pdf", tab_time)
f7 = pyplot.figure(7)

#MAX
graph_builder("Time in iteration", "Infected", "Maximum infection graph", 1, 10000, f"{out_path}max_graph_{out_name}.pdf", tab_max_infec, list(range(25, 400, 50)))
f6 = pyplot.figure(6)
histogram_builder("Maximum infected", "Density", "Maximum infected histogram", True, 10, f"{out_path}max_hist_{out_name}.pdf", tab_max_infec, list(range(25, 400, 50)))
f5 = pyplot.figure(5)
heatmap_builder(1000, list(range(0,10)), "Maximum infected heatmap", f"{out_path}max_heat_{out_name}.pdf", tab_max_infec)
f4 = pyplot.figure(4)

#MULTI-INFECTIONS
graph_builder("Samples", "Number of multi-infections", "Multi-infections graph", 1, 10000, f"{out_path}mult_graph_{out_name}.pdf", tab_mult_infec, list(range(25, 400, 50)))
f3 = pyplot.figure(3)
histogram_builder("Multi-infections", "Density", "Multi-infections histogram", False, 50, f"{out_path}mult_hist_{out_name}.pdf", tab_mult_infec, [])
f2 = pyplot.figure(2)
heatmap_builder(100, list(range(0,10)), "Multi-infections heatmap", f"{out_path}mult_heat_{out_name}.pdf", tab_mult_infec)
pyplot.show()
