...
filename = "cont_period_data.txt" #Nom du fichier qui permet de réaliser les graphiques

out_name = "cont_period" #Fichiers de sortie contenant ce nom, ainsi qu'un sous-dossier de ce nom
...
def heatmap_builder(size, drange, title, output, data):
	...
	pyplot.savefig(output)
#TIME
graph_builder("Samples", "Time in iteration", "Time graph", 1, 10000, f"{out_path}time_graph_{out_name}.pdf", tab_time, list(range(0, 350, 50)))
f9 = pyplot.figure(9)
histogram_builder("Time in iteration", "Density", "Time histogram", True, 20, f"{out_path}time_hist_{out_name}.pdf", tab_time, list(range(0, 200, 50)))
f8 = pyplot.figure(8)
heatmap_builder(40, list(range(0,5)), "Time heatmap", f"{out_path}time_heat_{out_name}.pdf", tab_time)
f7 = pyplot.figure(7)
...
f6 = pyplot.figure(6)
histogram_builder("Maximum infected", "Density", "Maximum infected histogram", True, 10, f"{out_path}max_hist_{out_name}.pdf", tab_max_infec, list(range(0, 1000, 200)))
f5 = pyplot.figure(5)
heatmap_builder(1000, list(range(0,15)), "Maximum infected heatmap", f"{out_path}max_heat_{out_name}.pdf", tab_max_infec)
f4 = pyplot.figure(4)

#MULTI-INFECTIONS
graph_builder("Samples", "Number of multi-infections", "Multi-infections graph", 1, 10000, f"{out_path}mult_graph_{out_name}.pdf", tab_mult_infec, list(range(0, 1000, 150)))
f3 = pyplot.figure(3)
histogram_builder("Multi-infections", "Density", "Multi-infections histogram", True, 1, f"{out_path}mult_hist_{out_name}.pdf", tab_mult_infec, list(range(0, 1000, 50)))
f2 = pyplot.figure(2)
heatmap_builder(40, list(range(0,5)), "Multi-infections heatmap", f"{out_path}mult_heat_{out_name}.pdf", tab_mult_infec)
pyplot.show()
